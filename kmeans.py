...
def distance(instance1, instance2):
    ...
    if(distMetric == "euclidean"):
        ...
        return cosine(instance1[1:], instance2[1:])
    elif(distMetric == "gen-jacccard"):
        top = 0
        bottom = 0
        for i in range(1, len(instance1)):
        ...
        raise ValueError('Invalid Distance Selected')
...
def drawPoints(canvas, instances, color, shape):
    ...
    for instance in instances:
        ...
        if (shape == "square"):
            ...
            canvas.create_text(x+15, y, fill=color, text=instance[0])
    canvas.update()
...
# dataset = [["X1",3,5],["X2",3,4],["X3",2,8],["X4",2,3],["X5",6,2],["X6",6,4],["X7",7,3],["X8",7,4],["X9",8,5],["X10",7,6]]
# "euclidean" or "manhattan" or "cosine" or "gen-jacccard"
distMetric = "gen-jacccard"

showDataset2D(dataset)
clustering = kmeans(dataset, 4, animation=True)
